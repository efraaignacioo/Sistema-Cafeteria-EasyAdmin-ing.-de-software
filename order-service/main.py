...
app = FastAPI(
    ...
    version="1.1.0" # Nueva versión con ID de mesa
)
...
class Order(Base):
    ...
    table_number = Column(Integer, index=True) ### NUEVO: Columna para el número de mesa

# --- Modelos Pydantic ---
class OrderBase(BaseModel):
    items: List[str]
    table_number: int ### NUEVO: Campo para recibir el número de mesa
...
class OrderResponse(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    table_number: int ### NUEVO: Campo para enviar el número de mesa
...
class TicketResponse(BaseModel):
    ...
    total: float
    issued_at: datetime
    table_number: int ### NUEVO: Campo para el ticket
...
async def crear_pedido(pedido: OrderCreate, db: Session = Depends(get_db)):
    ...
    # ### MODIFICADO: Añadimos table_number al crear el pedido
    db_order = Order(
...
def generar_ticket(order_id: int, db: Session = Depends(get_db)):
    ...
    ticket = TicketResponse(
        ...
        table_number=order_db.table_number ### NUEVO: Añadimos al ticket
    )
    return ticket
